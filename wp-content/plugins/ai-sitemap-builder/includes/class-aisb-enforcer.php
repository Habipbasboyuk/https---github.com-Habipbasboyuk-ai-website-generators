...
class AISB_Enforcer {
  ...
  public function enforce_structure_only($data): array {
    ...
    foreach ($data['sitemap'] as $i => $p) {
      if (($p['page_type'] ?? '') === 'Home' || ($p['slug'] ?? '') === 'home') {
        $home_index = $i;
        break;
      }
    }
    ...
    if ($home_index === -1) {
      array_unshift($data['sitemap'], [
        ...
        'priority'     => 'Core',
        'parent_slug'  => null,
        'page_purpose' => 'Main landing page for the website.',
        'sections'     => [],
        'seo'          => [
      ...
      if (!isset($data['sitemap'][$home_index]['sections']) || !is_array($data['sitemap'][$home_index]['sections'])) {
        $data['sitemap'][$home_index]['sections'] = [];
      }
    }
    ...
    foreach ($data['sitemap'] as $i => $p) {
      ...
      if (!isset($data['sitemap'][$i]['parent_slug']) ||
          $data['sitemap'][$i]['parent_slug'] === null ||
          $data['sitemap'][$i]['parent_slug'] === '') {
        $data['sitemap'][$i]['parent_slug'] = 'home';
      }
      if (!isset($data['sitemap'][$i]['sections']) || !is_array($data['sitemap'][$i]['sections'])) {
        $data['sitemap'][$i]['sections'] = [];
      }
    }
