...
class Search {
	...
	public static function get_search_template_criteria_post_ids( $template_id, $search_term ) {
		...
		if ( $has_post_fields || $has_meta_fields ) {
			// Prepare meta fields
	 ...
	 * @since 2.2
	 */
	public static function get_post_ids_by_combined_search( $search_fields, $meta_fields, $search_term, $filter_id, $query_id, $use_weight_score ) {
		if ( empty( $search_fields ) && empty( $meta_fields ) ) {
			return [];
		}
		...
		// Generate cache key
		$cache_key = 'bricks_combined_search_posts_' . md5( serialize( $search_fields ) . serialize( $meta_fields ) . $search_term . $filter_id . $query_id . ( $use_weight_score ? '_weighted' : '_unweighted' ) );
		...
		if ( $use_weight_score ) {
			$post_ids = self::get_weighted_post_ids( $search_fields, $meta_fields, $search_term, $filter_id, $query_id );
		} else {
			$post_ids = self::get_unweighted_post_ids( $search_fields, $meta_fields, $search_term, $filter_id, $query_id );
		}
	...
	private static function get_weighted_post_ids( $search_fields, $meta_fields, $search_term, $filter_id, $query_id ) {
		global $wpdb;
	...
	private static function get_unweighted_post_ids( $search_fields, $meta_fields, $search_term, $filter_id, $query_id ) {
		global $wpdb;
