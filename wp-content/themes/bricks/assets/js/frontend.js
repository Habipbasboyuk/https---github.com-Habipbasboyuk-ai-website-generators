...
const bricksToggleModeFn = new BricksFunction({
	parentNode: document,
	selector: '.brxe-toggle-mode .toggle',
	frontEndOnly: true,
	eachElement: (toggle) => {
		// Get current mode from body data-theme attribute
		let currentTheme = document.documentElement.dataset.theme

		// Update .is-active class for all theme toggles
		let modeToggles = bricksQuerySelectorAll(document, '.brxe-toggle-mode .toggle')

		modeToggles.forEach((modeToggle) => {
			if (modeToggle.classList.contains(currentTheme)) {
		...
		toggle.addEventListener('click', (e) => {
			...
			// Update body data-theme attribute & local storage
			if (currentTheme === 'dark') {
				document.documentElement.dataset.theme = 'light'
				localStorage.setItem('brx_mode', 'light')
			} else {
				document.documentElement.dataset.theme = 'dark'
				localStorage.setItem('brx_mode', 'dark')
			}
...
function bricksToggleMode() {
	...
	// Set defaultMode (light, dark, auto) according to project settings (@since 2.2)
	let savedTheme = localStorage.getItem('brx_mode')
	let defaultMode = window.bricksData?.defaultMode || 'light'
	...
	if (currentTheme === 'auto') {
		...
		window.bricksData.defaultMode = prefersDarkScheme ? 'dark' : 'light'
	}
	...
	// Run toggle-mode element function
